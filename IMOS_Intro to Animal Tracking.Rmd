...
Acoustic transmitters have been developed and commercially produced by a number of manufacturers. The IMOS Animal Tracking Facility utilises tracking technology primarily from [Inovasea](https://www.innovasea.com/fish-tracking/), however can support data obtained from a range of other systems. Transmitters developed by Inovasea range in size from 15 - 98mm, with the largest tags having a battery life of 10 years. 
...
![© Charlie Huveneers](Images/Tag Surgery 1.png) 

<br><br>

Larger animals, or animals difficult to handle can also be tagged externally using tags teathered to specialised dart tags. 
...
Acoustic telemetry has been successfully implemented on an incredibly diverse range of species and for diverse objectives ([Hussey et al., 2015 ](https://science.sciencemag.org/content/348/6240/1255642.abstract)). Acoustic telemetry includes both passive acoustic telemetry where a network of static receivers detects animals moving through their environment, and active acoustic tracking where an individual animal is followed by a boat with a mobile receiver.
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, out.width='100%', fig.cap = "Map: Historic and current locations from which data is available in the [IMOS ATF webapp](https://animaltracking.aodn.org.au)"}
library(mapview)
library(tidyverse)
library(sf)
library(lubridate)
...
library(vembedr)
vembedr::embed_youtube(id = "2mhisuvONg8", allowfullscreen = T) %>% 
