...
statinfo <-
  ...
  st_as_sf(coords = c("deployment_longitude", "deployment_latitude"), crs = 4326, remove = F)
  
statinfo %>% 
  leaflet() %>% 
  addProviderTiles(providers$Esri.WorldImagery) %>% 
  addMarkers(clusterOption = markerClusterOptions(removeOutsideVisibleBounds = T),
             popup = ~as.character(station_name),
             label = ~as.character(installation_name))
